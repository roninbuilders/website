...
The `useRNS` hook returns a value of type _string_ of the connected user's RNS or _undefined_ if the user is not connected ro does not own an RNS.

```tsx
import { useRNS } from '@roninbuilders/modal-wagmi/react'


export default function App(){

	const connectedRNS = useRNS()
